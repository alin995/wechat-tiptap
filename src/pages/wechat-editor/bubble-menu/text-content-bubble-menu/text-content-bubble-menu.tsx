	...
	return <>
		...
		>
			{actionStatus.tableGroup && <>
				{actionStatus.mergeCell &&
					<BubbleMenuItem icon={<MergeCellIcon className="w-5 h-5"/>} onClick={() => editor.chain().focus().mergeCells().run()}/>}
				{actionStatus.splitCell && <BubbleMenuItem
					icon={<SplitCellIcon className="w-5 h-5"/>}
					onClick={() => {
						if (isCellSelection(editor)) {
				...
				{actionStatus.removeTable &&
					<BubbleMenuItem icon={<DeleteIcon className="w-5 h-5"/>} onClick={() => editor.chain().focus().deleteSelection().run()}/>}
				{actionStatus.removeTableRow &&
					<BubbleMenuItem icon={<DeleteIcon className="w-5 h-5"/>} onClick={() => editor.chain().focus().deleteRow().run()}/>}
				{actionStatus.removeTableColumn &&
					<BubbleMenuItem icon={<DeleteIcon className="w-5 h-5"/>} onClick={() => editor.chain().focus().deleteColumn().run()}/>}
			</>}
